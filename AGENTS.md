...
4. **Salary and notice-period are a static refusal, not a drafted or stored answer.** (D17, retired 2026-08-15) `suggest` keyword-matches these before retrieval/gate/drafting and returns `refuse` — the user answers directly in the moment, and nothing is stored or auto-suggested for either field. There is no `sensitive_facts` table, no confirm/update lifecycle, and no per-write-path gate to replicate for a new insert path.
