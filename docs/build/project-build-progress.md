...
**Current state:** Phase 2 — S5a shipped (S1–S5a on `main`). S5b (ask path) is next.
...
- 2026-08-11 — **S5a completed.** Suggest endpoint — gate, draft, refuse merged to `main` as `ac561be` (branch `fm/jobibi-s5a-suggest`, PR #10). `supabase/migrations/20260811000000_gate_decisions.sql` adds `gate_decisions` (RLS, `question_norm`, `question_match`, `role_match`, `outcome`, `user_action`). Shared: `packages/shared/src/gate/{normalize,retrieve,gate}.ts` — two-axis gate `relative gap 0.12 + floor 0.28, role 0.35, bias to ask`, `decideGate` (code decides, never model), 6 vitest vs golden set. Edge Function `supabase/functions/suggest` — retrieve hybrid (gte-small vector + keyword, 0.7/0.3), gate (`ask→refuse` for S5a), log to `gate_decisions`, then Luna `gpt-5.6-luna` via strict JSON schema `answer + skeleton + sources` with `max_tokens 600` / `answer ≤900 chars` (D8/D14), `OPENAI_API_KEY` set on `kbpojtjemftqwgmrnbdq` and deployed `--use-api` (and `ingest` redeployed). Extension: `apps/extension/entrypoints/sidepanel/SuggestCard.tsx` per question (`Suggest` → amber refuse card or emerald draft card with `Copy answer`/`Copy skeleton`, `q/r` scores) + `JobStreetQuestions.tsx` integration (tab-aware). Verified: `shared` 56 passed (7 files), `extension` 5 passed, `tsc --noEmit` 0, `wxt build` 451 kB, `wxt check-manifest` ok.
