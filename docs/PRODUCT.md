...
3. **Progressive memory.** Day one, the memory bank holds only what the user uploads (resumes, cover letters, transcripts) plus a sixty-second intake covering the four facts every form asks for. Every completed application feeds submitted answers back into memory. After roughly 15–20 applications the assistant is attuned to the user's tone and best stories.
